...
for item in store['archive_uploads']:
    if item.get('file_name') == 'mobile-scorecard.heic':
        print('FOUND')
        ...
        print('raw_extracted_text', repr(item.get('raw_extracted_text')[:200]))
        print('extraction_template_keys', list(item.get('extraction_template', {}).keys()))
        break
else:
    print('NOT FOUND')
